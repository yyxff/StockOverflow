...
func TestRefillLessSeller(t *testing.T) {
	...
	sellers := NewSellerHeap("SPY", 2, 2)
	sellers.SetDB(dbm.Db)

	t1 := time.Now()
	t2 := t1.Add(2 * time.Second)
	sellers.SafePush(NewOrder("1", 1, decimal.NewFromFloat(3.0), t2))
...
func TestRefillLessSeller2(t *testing.T) {
	dbm := database.DatabaseMaster{
		ConnStr: server.GetDBConnStr(),
		DbName:  "stockoverflowtest",
	}
